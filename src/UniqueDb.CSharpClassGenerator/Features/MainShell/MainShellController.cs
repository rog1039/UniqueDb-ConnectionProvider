...
namespace UniqueDb.CSharpClassGenerator.Features.MainShell
{
    public class MainShellController
    {
		...
		public ObservableCollection<object> SqlQueryResults { get; } = new ObservableCollection<object>();
        ...
        private void ExecuteQuery()
        {
            try
            {
                var connFactory = DatabaseSelectionController.DbConnectionFactory.Value;
                var conn = connFactory();
                var cSharp = CSharpClassGeneratorFromAdoDataReader.GenerateClass(conn, SqlQuery.Value, ClassName.Value);
                GeneratedCSharpText.Value = cSharp;


                conn = connFactory();
                var cmd = new SqlCommand(SqlQuery.Value, (SqlConnection) conn);
        ...
        {
            var canExecute = DatabaseSelectionController.DbConnectionFactory.Value != null &&
                             !string.IsNullOrWhiteSpace(ClassName) && !string.IsNullOrWhiteSpace(SqlQuery);
            return canExecute;
        }
    }
