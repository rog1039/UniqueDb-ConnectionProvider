...
{
    private static string _sqlInformationSchemaColumn = 
        "SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'{0}' AND TABLE_CATALOG = '{1}'";
    
    private static string _sqlInformationSchemaTable = 
        "SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = N'{0}' AND TABLE_CATALOG = '{1}'";

    public static string GetInformationSchemaColumnsSqlQuery(SqlTableReference sqlTableReference)
    {
        var sql = string.Format(_sqlInformationSchemaColumn, 
                                sqlTableReference.TableName.Debracketize(), 
